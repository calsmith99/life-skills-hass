...
const waitForHAFrontend = () => {
  return new Promise((resolve) => {
    const checkFrontend = () => {
      // Basic check - if customElements and ha-card exist, we're good to go
      if (window.customElements && window.customElements.get('ha-card')) {
        ...
        if (window.lit) {
          ...
          console.log('Life Skills Card: Found LitElement via window.lit');
        ...
        if (haCard && haCard.prototype) {
          // Get LitElement from ha-card's prototype chain
          let proto = haCard.prototype;
          while (proto && proto.constructor && proto.constructor.name !== 'LitElement') {
            proto = Object.getPrototypeOf(proto);
          }
          
          if (proto && proto.constructor && proto.constructor.name === 'LitElement') {
            LitElement = proto.constructor;
            
            // Create simple template functions if not available globally
            html = window.html || ((strings, ...values) => {
              return { _$litType$: 1, strings, values };
            });
            css = window.css || ((strings, ...values) => {
              return { _$litType$: 2, strings, values };
            });
            
            console.log('Life Skills Card: Found LitElement via ha-card prototype');
            resolve({ LitElement, html, css });
            return;
          }
        }
        
        // Method 3: If we can't find LitElement but HA is loaded, just use a simple fallback
        console.log('Life Skills Card: Could not find LitElement, using HTMLElement fallback');
        resolve(null); // Signal to use HTMLElement fallback
        
      } else {
        // Still waiting for basic HA components
        setTimeout(checkFrontend, 200);
      }
    };
...
waitForHAFrontend().then((result) => {
  
  if (result && result.LitElement) {
    // Use LitElement implementation
    const { LitElement, html, css } = result;
  ...
  if (!window.customCards.find((c) => c.type === 'life-skills-card')) {
    ...
    // Fallback to HTMLElement implementation
    console.log('Life Skills Card: Using HTMLElement fallback');
    ...
    class LifeSkillsCard extends HTMLElement {
      constructor() {
        ...
        this._lastSkill = undefined;
        this._lastLevel = undefined;
        this._lastXp = undefined;
        this._lastLevelState = undefined;
        this._lastXpToNext = undefined;
      }
      ...
      setConfig(config) {
        ...
        if (!('skill' in config)) config.skill = '';
        this.config = config;
        this._lastSkill = undefined; // force re-render
        this.render();
      }
      ...
      render() {
        ...
        let level = 1, xp = 0, xpToNext = 0, skillName = '', levelEntityState = '', xpToNextEntityState = '';

        if (selectedSkill && this._hass.states && this._hass.states[selectedSkill]) {
          const xpEntity = this._hass.states[selectedSkill];
          xp = parseInt(xpEntity.state) || 0;
          const levelEntityId = selectedSkill.replace('number.', 'sensor.').replace('_xp', '_level');
          const levelEntity = this._hass.states[levelEntityId];
          level = levelEntity ? parseInt(levelEntity.state) || 1 : 1;
          levelEntityState = levelEntity ? levelEntity.state : '';
          const xpToNextEntityId = selectedSkill.replace('number.', 'sensor.').replace('_xp', '_xp_to_next');
          const xpToNextEntity = this._hass.states[xpToNextEntityId];
          xpToNext = xpToNextEntity ? parseInt(xpToNextEntity.state) || 0 : 0;
          xpToNextEntityState = xpToNextEntity ? xpToNextEntity.state : '';
          skillName = xpEntity.attributes && xpEntity.attributes.friendly_name ? xpEntity.attributes.friendly_name : 'Unknown Skill';
          if (skillName.endsWith(' XP')) {
            skillName = skillName.slice(0, -3);
          }
        }
        ...
        if (
          ...
          this._lastXp === xp &&
          this._lastLevelState === levelEntityState &&
          this._lastXpToNext === xpToNextEntityState
        ) {
          return;
        }
        ...
        if (selectedSkill && this._hass.states && this._hass.states[selectedSkill]) {
          ...
          const skillIcon = this._hass.states[selectedSkill].attributes && this._hass.states[selectedSkill].attributes.icon ? this._hass.states[selectedSkill].attributes.icon : 'mdi:star';
          skillContent = `
            <div class="skill-card-container">
              <div class="top-row">
                <div class="icon-section">
    ...
    class LifeSkillsCardEditor extends HTMLElement {
      ...
      render() {
        ...
        this.shadowRoot.innerHTML = `
          ...
          <div class="form-group">
            ...
            <select id="skill-select">
              <option value="">Select a skill...</option>
              ${skills.map(skill => `
                <option value="${skill.entity_id}" ${this.config.skill === skill.entity_id ? 'selected' : ''}>
                  ${skill.name}
                </option>
              `).join('')}
            </select>
        ...
        const select = this.shadowRoot.querySelector('#skill-select');
        if (select) {
          select.addEventListener('change', (ev) => {
            this._valueChanged('skill', ev.target.value);
          });
        }
      }
    ...
    console.log('Life Skills Card: Successfully initialized with HTMLElement fallback');
  }

  // Register the custom elements (works for both LitElement and HTMLElement)
  if (!customElements.get('life-skills-card')) {
    customElements.define('life-skills-card', LifeSkillsCard);
    console.log('Life Skills Card: Registered life-skills-card element');
  }
